...
TEMPLATES = {
    "react": {
        "dockerfile_template": "templates/react/Dockerfile.template",
        "config_file": "templates/reactjs/config.yaml",
        "default_port": 5173,
        "runtime": "node",
        "framework": "React.js"
    },
    "nextjs": {
        "dockerfile_template": "templates/nextjs/Dockerfile.template", 
        "config_file": "templates/nextjs/config.yaml",
        "default_port": 3000,
        "runtime": "node",
        "framework": "Next.js"
    },
    "fastapi": {
        "dockerfile_template": "templates/fastapi/Dockerfile.template",
        "config_file": "templates/fastapi/config.yaml", 
        "default_port": 8000,
        "runtime": "python",
        "framework": "FastAPI"
    }
...
def generate_template_dockerfile(template_name: str, base_image: str, config: Dict[str, Any]) -> str:
    ...
    if 'additional_packages' in config and config['additional_packages']:
        processed_config['additional_packages_install'] = f"RUN npm install -g {' '.join(config['additional_packages'])}"
    else:
        processed_config['additional_packages_install'] = ""
    
    # Handle VSCode extensions installation
    if 'vscode_extensions' in config and config['vscode_extensions']:
        extensions_cmd = "RUN for ext in " + " ".join(config['vscode_extensions']) + "; do \\\n"
        extensions_cmd += "        /opt/openvscode-server/bin/openvscode-server --install-extension $ext; \\\n"
        extensions_cmd += "    done"
        processed_config['vscode_extensions_install'] = extensions_cmd
    else:
        processed_config['vscode_extensions_install'] = ""
    
    # Handle packages for Python templates
    if 'packages' in config and config['packages']:
        processed_config['packages'] = " ".join(config['packages'])
    
    # Replace placeholders
    dockerfile_content = template_content.format(
        base_image=base_image,
        USERNAME="framelab",  # Default username
        **processed_config
    )
